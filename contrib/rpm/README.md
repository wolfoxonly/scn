...
The RPM spec file provided here is for StarChain-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://StarChain.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[StarChain git repository](https://github.com/StarChain/StarChain) but are not part of
...
Currently only `Source10` is used, for the example `StarChain.conf` file.
...
in the StarChain git repository. At present only an SVG version of the StarChain
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the StarChain
...
needed when 0.12.1 is released, a proper fix is already in the StarChain git
master branch.
...
idea to build StarChain Core against a static version of that library compiled
...
The `StarChain.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `StarChain-server` package. At this point, most RPM based
...
The `StarChain-server` package should have SELinux support. How to properly do
...
StarChain-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/StarChain/). He has been
packaging StarChain for Fedora at least since 2012.
...
4. I renamed the `StarChain` package that contains the Qt GUI to `StarChain-core` as
that appears to be how the general population refers to it, in contrast to
`StarChain-xt` or `StarChain-classic`. I wanted to make sure the general population
