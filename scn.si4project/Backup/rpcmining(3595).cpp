...
Value setgenerate(const Array& params, bool fHelp)
{
    ...
    GenerateStarchains(fGenerate, pwalletMain);
    return Value::null;
}
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Starchain is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Starchain is downloading blocks...");
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "See https://en.Starchain.it/wiki/BIP_0022 for full specification.");
