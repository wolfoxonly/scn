...
###   bitcoincore.org and Starchain.org and compares them.
...
WORKINGDIR="/tmp/Starchain_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitcoincore.org"
HOST2="https://Starchain.org"
BASEDIR="/bin/"
VERSIONPREFIX="Starchain-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'Starchain-',
...
if [ $? -ne 0 ]; then
   echo "Starchain.org failed to provide signature file, but bitcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "Starchain.org and bitcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Starchain Core binary release signing key installed?"
   fi
